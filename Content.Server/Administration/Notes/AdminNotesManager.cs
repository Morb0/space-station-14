...
public sealed class AdminNotesManager : IAdminNotesManager, IPostInjectInit
{
    ...
    public async Task AddAdminRemark(ICommonSession? createdBy, Guid player, NoteType type, string message, NoteSeverity? severity, bool secret, DateTime? expiryTime)
    {
        ...
        var sb = new StringBuilder($"{createdBy?.Name ?? "System"} added a");
        ...
        switch (type)
        {
            case NoteType.Note:
                ...
                noteId = await _db.AddAdminNote(roundId, player, playtime, message, severity.Value, secret, createdBy?.UserId ?? new NetUserId(), createdAt, expiryTime);
                break;
            case NoteType.Watchlist:
                secret = true;
                noteId = await _db.AddAdminWatchlist(roundId, player, playtime, message, createdBy?.UserId ?? new NetUserId(), createdAt, expiryTime);
                break;
            case NoteType.Message:
                noteId = await _db.AddAdminMessage(roundId, player, playtime, message, createdBy?.UserId ?? new NetUserId(), createdAt, expiryTime);
                seen = false;
                break;
        ...
        var note = new SharedAdminNote(
            ...
            severity,
            secret,
            createdBy?.Name ?? Loc.GetString("system-user"),
            createdBy?.Name ?? string.Empty,
            createdAt,
            createdAt,
            expiryTime,
            null,
